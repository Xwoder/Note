...
* 总体相关系数用符号 $ \rho $ 表示；样本相关系数用符号 $ r $ 表示
...
#### 残差平方和 / `Sum of Squared Estimate of Errors`

$$
SSE = \sum e_i^2 = \sum (y_i - \hat{y}_i)^2
$$

####  均方残差 / `Mean Squared Error`

缩写为 `MSE`
$$
\mathrm{MSE} = \frac{\mathrm{SSE}}{n-2}
$$

#### 估计标准误差

$$
s_e = \sqrt{\mathrm{MSE}} = \sqrt{\dfrac{\mathrm{SSE}}{n-2}} = \sqrt{\dfrac{\sum e_i^2}{n-2}} = \sqrt{\dfrac{\sum (y_i - \hat{y}_i)^2}{n-2}}
$$

$ s_e $  越小，说明 $ y $ 的实际值与回归值越接近，回归直线的代表性越好；$ s_e $ 值越大，说明 $ y $ 的实际值与回归值越偏离，回归直线的代表性越不好。

$ s = 0 $，说明y的实际值与回归值没有偏差，回归值完全正确。
...
|       缩写       |                全称                |         中文名          |                             公式                             |
| :--------------: | :--------------------------------: | :---------------------: | :----------------------------------------------------------: |
| $ \mathrm{SST} $ |     `Sum of Square for Total`      |        总平方和         | $$ \sum\limits_{i=1}^{n}\left(y_{i}-\bar{y}_i\right)^{2} $$  |
| $ \mathrm{SSR} $ | `Sum of Squares Due to Regression` |       回归平方和        | $$\sum\limits_{i=1}^{n}\left(\widehat{y}_{i}-\bar{y}_i\right)^{2}$$ |
| $ \mathrm{SSE} $ |     `Sum of Squares for Error`     | 残差平方和 / 误差平方和 | $$\sum\limits_{i=1}^{n}\left(y_{i}-\widehat{y}_i\right)^{2}$$ |

判定系数 / `coefficient of determination`
$$
R^2 = \frac{SSR}{SSE} = 1 - \frac{\mathrm{SSE}}{\mathrm{SST}}
$$
...
* $ R^2 $ 越接近于1，表明回归平方和占总平方和的比例越大，回归直线与各观测点越接近，用 $x$ 的变化来解释 $y$ 值变差的部分就越多，回归直线的你和程度就越好
* $ R^2 $ 越接近 $ 0 $，回归直线的拟合程度就越差

$$
\begin{array}{l}
\mathrm{MSR}=\dfrac{\mathrm{SSR}}{1} \\
\mathrm{MSE}=\dfrac{\mathrm{SSE}}{n-2}
\end{array}
$$

线性关系检验的统计量
$$
F=\dfrac{\mathrm{SSR} / 1}{\mathrm{SSE} /(n-2)}=\dfrac{\mathrm{MSR}}{\mathrm{MSE}} \sim F \left(1, n-2\right)
$$